...
contract bet is ATM, Ownable {
    ...
    function createBet (string memory _name, uint _teamId) external payable {       
        require (msg.sender != conOwner, "Owner not eligible to bet");
        require (numBetsAddress[msg.sender] == 0, "You cannot bet more than once");
        require (msg.value > 0.01 ether, "Insufficient amount betted");
        deposit();
